// JS pour la portee 
//ATTENTION evitons de déclarer plusieurs variables dans différents espaces en utilisant un même nom : ici on le fait pour la démo !!!
function porteNew () {
    let xLet = 1;
    var yVar = 2;

    if (true) {
        let xLet = 500;// let n'aura pas une portée au-delà de ce bloc
        let yVar = 200;// var aura une portée au delà de ce bloc

        document.getElementById('p1').innerHTML = 'la variable let "xLet" dans le if = ' + xLet;

        document.getElementById('p2').innerHTML = 'la variable let "xLet" dans le if = ' + yVar;
    }// fin if

    document.getElementById('p3').innerHTML = 'la variable let "xLet" hors du if = ' + xLet;

    document.getElementById('p4').innerHTML = 'la variable let "xLet" hors du if = ' + yVar;//yVar affichera la   dernière valeur déclarée car var a une portée élargie
}// fin fonction
porteNew()
...
function portee3() {// les variables locales de la fonction ont le même nom que les globales
    ...
    document.getElementById('p7').innerHTML = 'Depuis la fonction <code>portee3()</code> : a ' + x + ' et y ' + y;
}
// Appel des fonctions
portee1();
portee2();
